...
import { RotateCcw, Play, XOctagon, DollarSign, AlertTriangle, AlertOctagon, Users, User, Check, Trophy, Lock, Star } from 'lucide-react';
      ...
      return (
        <div className="absolute inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center px-4">
          <div className="aaa-panel aaa-panel-strong p-8 max-w-sm w-full text-center relative overflow-hidden">
            <div className="absolute inset-0 bg-[repeating-linear-gradient(45deg,transparent,transparent_10px,rgba(255,255,255,0.03)_10px,rgba(255,255,255,0.03)_20px)] pointer-events-none opacity-70"></div>
  ...
  return (
      ...
      >
        ...
        <div className="absolute inset-0 z-0 bg-[linear-gradient(rgba(148,163,184,0.08)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.08)_1px,transparent_1px)] bg-[size:52px_52px] opacity-25"></div>
         ...
         <div className={`aaa-panel aaa-panel-strong z-50 flex flex-col md:flex-row gap-6 p-4 md:p-8 relative max-w-6xl w-[96vw] max-h-[90vh] overflow-y-auto ${menuPanelOffsetClass}`}>
                ...
                <div className="grid gap-3">
                   {scenarios.map(scene => {
                      ...
                      <button 
                         ...
                         className={`p-4 border rounded-lg text-left transition-all relative overflow-hidden group
                            ${selectedScenarioId === scene.id 
                               ? 'border-cyan-400 bg-cyan-900/30 shadow-[0_0_24px_rgba(6,182,212,0.28)]' 
                      ...
                      >
                         ...
                         {isLocked && (
                            <div className="text-[10px] text-amber-400 font-mono mb-2">{lockReason}</div>
                         )}
                         <div className="flex justify-between items-center text-[10px] font-mono border-t border-dashed border-white/10 pt-2">
                            ...
                            {highScore > 0 && (
                              <span className="text-cyan-300">BEST {highScore.toLocaleString()}</span>
                            )}
                         </div>
            ...
            <div className="w-full md:w-80 flex flex-col gap-6 border-l border-slate-700/60 pl-0 md:pl-6 pt-4 md:pt-0">
                            ...
                            <button
                                ...
                                className={`w-full p-3 border rounded-lg flex items-center gap-3 transition-all relative
                                    ${selectedCrewId === crew.id 
                                        ? 'border-emerald-400 bg-emerald-900/20 shadow-[0_0_16px_rgba(16,185,129,0.25)]' 
                      ...
                      {modes.map(mode => (
                        <button
                          ...
                          className={`text-left p-2 rounded border transition-all ${
                            gameMode === mode.id
                              ? 'border-cyan-400 bg-cyan-900/30 shadow-[0_0_14px_rgba(34,211,238,0.24)]'
                              : 'border-slate-700 bg-slate-900/60 hover:bg-slate-800/70'
                          }`}
                        >
                ...
                <div className="mt-auto pt-8">
                    <Button variant="primary" onClick={() => customStart && selectedScenarioId && customStart(selectedScenarioId, selectedCrewId, gameMode)} className="w-full text-xl py-6">
